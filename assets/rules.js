...
/**
 * Catégories des activités ponctuelles (dates variables, décidées au cas par
 * cas — pas de règle de jour de semaine). Un membre s'auto-déclare présent à
...
const CATEGORIES_EVENEMENTS = [
  ...
  { code: "PROMESSE_ST_ESPRIT",  label: "Promesse du Saint-Esprit" },
  { code: "GLORY",               label: "GLORY" },
  { code: "MSAJ_CVAC",           label: "MSAJ / C'VAC" },
  { code: "EXCURSION_GALA",      label: "Excursion / Gala de clôture" },
  { code: "DLSDS",               label: "DLSDS" },
  { code: "KERMESSE",            label: "Kermesse" },
  { code: "VISA_JERICHO",        label: "Visa à Jéricho" },
  { code: "TRONE_GLOIRE",        label: "Trône de Gloire" },
  { code: "BRISE_CARMEL",        label: "Brise du Carmel" },
  { code: "RENCONTRE_MENSUELLE", label: "Rencontre mensuelle" },
  { code: "AUTRE",               label: "Autre" }
];

function libelleCategorieEvenement(code) {
  const c = CATEGORIES_EVENEMENTS.find(c => c.code === code);
...
if (typeof module !== "undefined") {
  module.exports = {
    ACTIVITES, MOIS_FR, JOURS_FERIES_CI, CATEGORIES_EVENEMENTS, libelleCategorieEvenement,
